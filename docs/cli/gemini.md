...
**Binary:** `/home/dev/.bun/bin/gemini`
...
Active Google account: `redacted@example.com` (from `~/.gemini/google_accounts.json`).
...
Current account tier: **Standard (Paid)** (Google AI Pro subscription via `redacted@example.com`).
...
```json
// /home/dev/mycelium-v2/.gemini/settings.json
{
  "mcpServers": {
    "playwright": {
      "command": "npx",
